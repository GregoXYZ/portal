 ...
 */
(function($){
    var winJQ, lastMouseX, lastMouseY, zIndex = 100000, minH, minW = 160, newWinOffset = 50, isIE = navigator.userAgent.match(/MSIE/), template = '<div class=window><div class=windowtitlebar><img src=/favicon.ico width=16 height=16 class=titlebaricon><div class=titlebartext></div><div class=horizbuts><div class=minimizebut title=minimize></div><div class=restorebut title=restore></div><div class=maximizebut title=maximize></div><div class=closebut title=close></div></div></div><div class=windowcontent></div><div class=resizer-tl></div><div class=resizer-t></div><div class=resizer-tr></div><div class=resizer-r></div><div class=resizer-br></div><div class=resizer-b></div><div class=resizer-bl></div><div class=resizer-l></div></div>', resizerP = $('<div id=resizerproxy>').mousemove(function(e){
        if (resizeMask & 1) {
        ...
        winJQ = undefined;
        return false;
    }), resizerC = $('<div>').appendTo(resizerP), resizerPos, resizeMask, // top=1,right=2,bottom=4,left=8
 onStartResize = function(e){
        ...
        if (type.match(/l/)) 
            ...
            if (type.match(/r/)) 
                resizeMask |= 2;
        winJQ = $(this).closest('.window').removeClass('minimized').removeClass('maximized').removeData('oldPos');
        ...
        resizerP.show();
        return false;
    }, moverP = $('<div id=moverproxy>').mousemove(function(e){
        ...
        width: 0,
        height: 0
    }, moverC = $('<div>').appendTo(moverP), lastClick = 0, onStartMove = function(e){
    ...
    savePos = function(w){
        ...
        $(this).closest('.window').WM_raise();
        return false;
    }, onClickMinimizeBut = function(){
        $(this).closest('.window').WM_minimize();
        return false;
    }, onClickRestoreBut = function(){
        $(this).closest('.window').WM_restore();
        return false;
    }, onClickMaximizeBut = function(){
        $(this).closest('.window').WM_maximize();
        return false;
    }, onClickCloseBut = function(){
        $(this).closest('.window').WM_close();
        return false;
    ...
    $.fn.WM_minimize = function(){
        this.each(function(){
            ...
            w.css({
                ...
                height: minH,
                zIndex: 100000
            }).removeClass('focused').addClass('minimized');
        });
        $.WM_retileMin();
        return this;
    };
    ...
    $.fn.WM_restore = function(){
        ...
        this.each(function(){
            ...
            if (w.hasClass('minimized')) {
                ...
                w.removeClass('maximized');
            w.css(w.data('oldPos')).removeData('oldPos').WM_ensure_viewable()
        });
        if (retile) 
            $.WM_retileMin();
        this.WM_raise();
    ...
    $.fn.WM_raise = function(){
        ...
        if (w.length == 0 || w.hasClass('focused')) 
            return this;
    ...
    $.WM_open = function(lnk, target, opts){
        ...
        // if content is url, load in iframe
            ...
            if (typeof lnk == 'string') {
                ...
                '<div class=iframecover></div>').appendTo(w.find('.windowcontent'));
            }
            
            // else let jquery append it
            else 
                w.find('.windowcontent').append(lnk);
