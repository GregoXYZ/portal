package presentation.util.images;
...
public class ProcesadorImagenes {
	 ...
	 */
	private final Log logger = LogFactory.getLog(ProcesadorImagenes.class);
	...
	/**
	 * Factor de convolucion que aplicamos para los algoritmos de suavizado.
	 */
	...
	/**
	 * Listado de formatos a los que debe aplicarse convolucion por sus perdidas
	 * en caso de reducciones muy pronunciadas.
	 */
	protected static List<String> listadoFormatosFiltroReduccionRuido;

	/* Metodos */
	/**
	 * Constructor de la clase. Carga opciones de renderizado por defecto,
	 * tendiendo a la calidad.
	 */
	 ...
	 * 
	 * @param opcionesRenderizadoPropias
	 *            Opciones propias de renderizado
	 */
	public ProcesadorImagenes(Map<Key, Object> opcionesRenderizadoPropias) {
		opcionesRenderizadoImagenes = new RenderingHints(
				opcionesRenderizadoPropias);
		cargarListadoFormatosReduccionRuido();
	}
	...
	private void cargarListadoFormatosReduccionRuido() {
		...
		listadoFormatosFiltroReduccionRuido
				.add(CodigosFormatosImagenes.CODIGO_FORMATO_JPEG_MAYUSC);
		listadoFormatosFiltroReduccionRuido
				.add(CodigosFormatosImagenes.CODIGO_FORMATO_JPEG_MINUSC);
		listadoFormatosFiltroReduccionRuido
				.add(CodigosFormatosImagenes.CODIGO_FORMATO_JPG_MAYUSC);
		listadoFormatosFiltroReduccionRuido
				.add(CodigosFormatosImagenes.CODIGO_FORMATO_JPG_MINUSC);
		listadoFormatosFiltroReduccionRuido
				.add(CodigosFormatosImagenes.CODIGO_FORMATO_BMP_MINUSC);
		listadoFormatosFiltroReduccionRuido
				.add(CodigosFormatosImagenes.CODIGO_FORMATO_BMP_MAYUSC);
		listadoFormatosFiltroReduccionRuido
				.add(CodigosFormatosImagenes.CODIGO_FORMATO_WBMP_MINUSC);
		listadoFormatosFiltroReduccionRuido
				.add(CodigosFormatosImagenes.CODIGO_FORMATO_WBMP_MAYUSC);
	}

	/**
	 * Metodo que analiza si un formato dado como parametro requiere la
	 * aplicacion de algoritmos de reduccion de ruido si es una reduccion
	 * sensible.
	 * 
	 * @param formato
	 *            Formato sobre el que analizamos
	...
	/**
	 * Metodo que genera una serie de valores por defecto para las opciones de
	 * renderizado de las imagenes.
	 */
	private void cargarOpcionesRenderizadoDefecto() {

		/*
		 * Cargo las opciones de renderizado por defecto para la clase: en
		 * general, en este caso se tiende a la calidad de las imagenes
		 * generadas
		 */
		opcionesRenderizadoImagenes = new RenderingHints(
	 ...
	 * 
	 * @param maximoAncho
	 *            maximo tama�o para el ancho de la nueva imagen
	 * @param maximoAlto
	 *            maximo tama�o para el alto de la nueva imagen
	 * @param formato
	 *            Formato de la imagen. Determina los filtros a aplicar
	 * @param imagen
	 *            Imagen original que vamos a escalar
	...
	public BufferedImage escalarATamanyo(final File ficheroImagen,
		...
		{
			logger.warn("El archivo " + ficheroImagen.getAbsolutePath() + " no existe.");
			return null;
		}
	}
	 ...
	 * @param srcImg
	 *            una imagen en formato BufferedImage
	 * @param formatoOrigen
	 *            Formato de la imagen. Determina los filtros a aplicar
	...
	public BufferedImage escalar(final Double factorEscala,
		...
		if (factorEscala == 1) {
			...
			BufferedImage copia = new BufferedImage(srcImg.getWidth(), srcImg
					.getHeight(), tipoFormatoBufferedReader);
		...
		AffineTransformOp op = new AffineTransformOp(AffineTransform
				.getScaleInstance(factorEscala, factorEscala),
				opcionesRenderizadoImagenes);
		BufferedImage resultadoFiltro = op.filter(filtroInicial, null);

		/*
		 * Balanceo entre elementos BufferedImage para eliminar canales de
		 * transparencia extras, si hay
		 */
		BufferedImage biConversion = new BufferedImage(resultadoFiltro
				.getWidth(), resultadoFiltro.getHeight(),
				tipoFormatoBufferedReader);
	 ...
	 * @throws IOException
	 *             Excepciones de entrada / salida generales
	 */
	public Boolean salvarImagen(final BufferedImage imagen,
		...
		if (imagen != null && rutaFichero != null && formato != null) {
			ImageIO.write(imagen, formato, f);
			return true;
		} else {
	...
	/**
	 * Metodo que crea un objeto de tipo Kernel para aplicar en posteriores
	 * transformaciones.
	 * 
	 * @param factorEscala
	 *            Factor de escala que tiene la imagen
