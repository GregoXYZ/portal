...
import presentation.ui.upload.MonitoredDiskFileItemFactory;
import presentation.ui.upload.UploadListener;
...
public class ExtendedMultiPartRequestHandler implements MultipartRequestHandler {
	...
	protected static Log logger = LogFactory
			.getLog(ExtendedMultiPartRequestHandler.class);
	...
	private long maxFileSize(HttpServletRequest request, long maxLimit)
			throws BusinessException {
		HttpSession session = request.getSession();
		UserInfo user = (UserInfo) session.getAttribute("user");

		CuotasBO cuotaBO = (CuotasBO) SpringUtil.getInstance().getBean(
				"CuotasBO");
		...
		if (cuota == null)
			...
			return cuota.getCuoCuotaFile() > maxLimit ? maxLimit : cuota
					.getCuoCuotaFile();
		}
	}
	 ...
	 */
	public Hashtable<String, FormFile> getFileElements() {
	...
	public void rollback() {
		Iterator<FormFile> iter = elementsFile.values().iterator();
	...
	protected String getRepositoryPath(ModuleConfig mc) {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("File upload temp dir: " + tempDir);
		}
	 ...
	 */
	private static class CommonsFormFile implements FormFile, Serializable {

		/**
		 * 
		 */
		private static final long serialVersionUID = 5034885754804296904L;
		/**
